...
<form id="form" action="<?php echo base_url("initialisation/generalisation") ?>" method="post">
    <?php
    $a = $nb_variable + 1;
    for ($i = 0; $i < $nb_equation; $i++) {
        for ($j = 0; $j < $nb_variable - 1 ; $j++) {
            ?>
            <input type="text" name="<?php echo $i . $j ?>" class="form-control" class="isa">x<?php echo $j + 1 ?> +
        <?php
        }
        ?>
